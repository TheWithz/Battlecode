...
public class ArchonRobot extends BaseRobot {
    private ArrayList<RobotType> buildRobotTypes = new ArrayList<>();
    private RobotType[] possibleBots = {RobotType.SCOUT, RobotType.GUARD, RobotType.SOLDIER, RobotType.TURRET, RobotType.VIPER};

    int heiarchy = -1;

    public ArchonRobot(RobotController rc) {
        super(rc);
        Utility.fillRobotTypes(buildRobotTypes);
    }
    ...
    private void tryToConvert() throws GameActionException {
    ...
    private void tryToHeal() throws GameActionException {
    ...
    private void tryToBuild() throws GameActionException {
        RobotType robot = buildRobotTypes.get(random.nextInt(buildRobotTypes.size()));
        for (Direction d : Direction.values()) {
            if (rc.canBuild(d, robot)) {
                if (rc.isCoreReady()) {
                    rc.build(d, robot);
                    buildRobotTypes.remove(robot);
                    for (RobotType type : possibleBots) {
                        if (type != robot) {
                            buildRobotTypes.add(type);
                        }
                    }
                    if (buildRobotTypes.size() >= 50) {
                        Utility.fillRobotTypes(buildRobotTypes);
                    }
